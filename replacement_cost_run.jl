#replacement_cost_run_new
#attenuated lcm.surge in initial protection
...
@inline my_initial_protection_table(dike_unitcost :: Tuple{Float64, Float64}) = function (lcm::LocalCoastalImpactModel)
    ...
    attenuation_heigth = ((lcm_attenuated.data.saltmarshes_width .* saltmarshes_attenuation) .+ (lcm_attenuated.data.tidalflats_width .* tidalflats_attenuation) .+ (lcm_attenuated.data.mangroves_width .* mangroves_attenuation))
    attenuated_mu = (lcm_attenuated.surge_model.µ .- attenuation_heigth)
    simga = lcm_attenuated.surge_model.σ
    xi = lcm_attenuated.surge_model.ξ
    ...
    local_popdens = if (local_exposure_attenuated[1]>0) local_exposure_attenuated[2][1]/local_exposure_attenuated[1] else 0.0 end
  ...
  #  println("Hello! I'm ", lcm.id, ". My popdens is ",local_popdens," and my local gdpc is ",lcm.data.local_gdpc)
    ...
    if lcm.protection_level > 0
      lcm.data.sea_dike_heigth=quantile(lcm_attenuated.surge_model,1-1/lcm.protection_level)
  #=    
...
#run of replacement costs (difference in dike heights) initial protection (seadikes)
for times in partition([2015, 2015], 2, 1)
    ...
    for (level, data) in output
        CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_high_actual" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_medium_actual" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_low_actual" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_high_10" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_medium_10" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_low_10" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_high_20" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_medium_20" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_low_20" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_high_30" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_medium_30" * level * ".csv", data)
        #CSV.write("$(ENV["DIVA_DATA"])/dataset_global/outputs/paper_wetlands_value/initial_protection_replacement_cost/RC_low_30" * level * ".csv", data)        
     empty!(data)
    end
end
